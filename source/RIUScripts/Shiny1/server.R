...
shinyServer(function(input, output, session) {
  ...
  isolate({
	...
	if (presentSettingRv$bu == "107") {
		presentSettingRv$current <- as.Date("2019-02-26")
	}
  ...
  periodChanged <- observeEvent(
    {
      ...
      avgStatsTitle <- switch(presentButtonRv$avgStatsType,
                              "R" = "Average Recency (days) <br/>最近平均回访日数",
                              "F" = "Average Orders <br/>平均交易量",
                              "M" = "Average Spending ($) <br/>平均支出")
      output$avgStatsTitle <- renderUI(HTML(avgStatsTitle))
      ...
      output$noOfCustomerByStatsTitle <- 
        if (presentButtonRv$noOfCustomerByStatsType == "F") {
          renderUI(HTML("Number of Customers by Orders <br/>按照交易次数统计客户数量"))
        } else {
          renderUI(HTML("Number of Customers by Total Spending <br/>按照支出金额统计客户数量"))
        }
